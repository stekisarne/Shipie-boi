...
public class shipMovements : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        randomSprite = Random.Range(1, 4); //randomSprite ett slumpmässigt värde mellan 1 och 3

        if (randomSprite == 1) //kollar om randomSprite = 1
        {
            shipRend.sprite = sprite1; //sätter spriten till sprite1
        }

        else if (randomSprite == 2) //kollar om randomSprite = 2
        {
            shipRend.sprite = sprite2; //sätter spriten till sprite2
        }

        else
        {
            shipRend.sprite = sprite3; //sätter spriten till sprite3
        }
    }
    ...
    void Update()
    {
        ...
        forwardSpeed = forwardSpeed + 0.05f * Time.deltaTime;

        rightRotation = rightRotation - 1f * Time.deltaTime;

        leftRotation = leftRotation + 1f * Time.deltaTime;
    }
}
